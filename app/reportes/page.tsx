...
type Tab = 'diario' | 'mensual' | 'danos' | 'bancos' | 'objetos_perdidos' | 'departamentos'
...
export default function ReportesPage() {
  ...
  // Estado para buscador de objetos perdidos
  const [queryObjetos, setQueryObjetos] = useState('')
  const [tipoObjeto, setTipoObjeto] = useState('')
  const [fechaInicioObjetos, setFechaInicioObjetos] = useState('')
  const [fechaFinObjetos, setFechaFinObjetos] = useState('')
  const [datosObjetos, setDatosObjetos] = useState<any[]>([])
  const [statsObjetos, setStatsObjetos] = useState<any>(null)
  ...
  const handleBuscarObjetos = async () => {
    setCargandoObjetos(true)
    setError(null)
    try {
      const params = new URLSearchParams()
      if (queryObjetos) params.append('query', queryObjetos)
      if (tipoObjeto) params.append('tipo', tipoObjeto)
      if (fechaInicioObjetos) params.append('fechaInicio', fechaInicioObjetos)
      if (fechaFinObjetos) params.append('fechaFin', fechaFinObjetos)

      const response = await fetch(`/api/reportes/objetos-perdidos?${params.toString()}`)
      const data = await response.json()

      if (!response.ok) {
        throw new Error(data.error || 'Error al buscar objetos perdidos')
      }
      setDatosObjetos(data.objetos || [])
      setStatsObjetos(data.stats || null)
    } catch (e) {
      console.error(e)
      setError(e instanceof Error ? e.message : 'Error al buscar objetos perdidos')
    } finally {
  ...
  const handleBuscarDepartamentos = async () => {
    ...
    try {
      const params = new URLSearchParams()
      params.append('periodo', periodoDepto)
      params.append('fecha', fechaDepto)
      params.append('mes', mesDepto)
      params.append('año', añoDepto)
      if (oficinaDepto) {
        params.append('oficina', oficinaDepto)
      }

      const response = await fetch(`/api/reportes/departamentos?${params.toString()}`)
      const data = await response.json()

      if (!response.ok) {
        throw new Error(data.error || 'Error al obtener reporte por departamentos')
      }

      setDatosDepartamentos(data)

      if (data.length === 0) {
        setError('No se encontraron denuncias remitidas a departamentos en el periodo seleccionado.')
      }
    } catch (error) {
      console.error('Error:', error)
      const errorMessage = error instanceof Error ? error.message : 'Error al cargar reporte por departamentos'
      setError(errorMessage)
    } finally {
      setCargando(false)
    }
  ...
  const handleLimpiarFiltros = () => {
    ...
    } else if (activeTab === 'objetos_perdidos') {
      setQueryObjetos('')
      setTipoObjeto('')
      setFechaInicioObjetos('')
      setFechaFinObjetos('')
      setDatosObjetos([])
    ...
    <MainLayout>
      <div className="min-h-screen bg-[#f8fafc] p-4 sm:p-6 lg:p-8 font-sans">
        <div className="max-w-7xl mx-auto">
          ...
          <div className="bg-white rounded-2xl shadow-xl shadow-blue-900/5 border border-slate-100 p-6 mb-8 animate-in fade-in slide-in-from-bottom-4 relative overflow-hidden">
            ...
            <div className="flex flex-col lg:flex-row items-end gap-6">
              {activeTab === 'objetos_perdidos' ? (
                <div className="flex-1 w-full lg:w-auto grid grid-cols-1 md:grid-cols-4 gap-6 text-left">
                  <div className="md:col-span-2">
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">
                      Buscar por texto (Ej: Chapa, Marca, Nro, Titular...)
                    </label>
                    <div className="relative group">
                      <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                        <Search className="h-3.5 w-3.5 text-slate-400 group-hover:text-[#002147] transition-colors" />
                      </div>
                      <input
                        type="text"
                        placeholder="Escriba palabra clave..."
                        value={queryObjetos}
                        onChange={(e) => setQueryObjetos(e.target.value)}
                  ...
                  <div>
                    ...
                    <select
                      ...
                      className="block w-full px-4 py-2.5 bg-slate-50 border border-slate-200 text-[#002147] text-xs font-bold rounded-xl focus:ring-4 focus:ring-blue-50 focus:border-blue-200 transition-all outline-none"
                    >
                      ...
                      <option value="celular">Celular / Teléfono Móvil</option>
                      <option value="contrato">Contrato / Documento Privado</option>
                      <option value="chapa_vehiculo">Chapa de vehículo</option>
                      <option value="otro_objeto">Otros Objetos / Documentos</option>
                    </select>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      ...
                      <input
                        type="date"
                        value={fechaInicioObjetos}
                        onChange={(e) => setFechaInicioObjetos(e.target.value)}
                    ...
                    <div>
                      ...
                      <input
                        type="date"
                        value={fechaFinObjetos}
                        onChange={(e) => setFechaFinObjetos(e.target.value)}
                        className="block w-full px-3 py-2.5 bg-slate-50 border border-slate-200 text-[#002147] text-xs font-bold rounded-xl focus:ring-4 focus:ring-blue-50 focus:border-blue-200 transition-all outline-none"
                      />
                    </div>
                  </div>
                ...
                <div className="flex-1 w-full lg:w-auto grid grid-cols-1 md:grid-cols-4 gap-6 text-left">
                  {/* Selector de Periodo */}
                  <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Periodo</label>
                    <select
                      value={periodoDepto}
                      onChange={(e) => setPeriodoDepto(e.target.value as any)}
                  ...
                  {/* Filtro de Fecha (Solo para Diario o Semanal) */}
                  {(periodoDepto === 'diario' || periodoDepto === 'semanal') && (
                    <div>
                      <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">
                        {periodoDepto === 'diario' ? 'Fecha Específica' : 'Fecha de la Semana'}
                      </label>
                      <input
                        type="date"
                        value={fechaDepto}
                        onChange={(e) => setFechaDepto(e.target.value)}
                        className="block w-full px-3 py-2 bg-slate-50 border border-slate-200 text-[#002147] text-xs font-bold rounded-xl focus:ring-4 focus:ring-blue-50 focus:border-blue-200 transition-all outline-none"
                      />
                    </div>
                  )}

                  {/* Filtro de Mes y Año (Solo para Mensual) */}
                  {periodoDepto === 'mensual' && (
                    <div className="grid grid-cols-2 gap-4 col-span-1 md:col-span-2">
                      <div>
                        <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Mes</label>
                        <select
                          value={mesDepto}
                          onChange={(e) => setMesDepto(e.target.value)}
                          className="block w-full px-3 py-2 bg-slate-50 border border-slate-200 text-[#002147] text-xs font-bold rounded-xl focus:ring-4 focus:ring-blue-50 focus:border-blue-200 transition-all outline-none cursor-pointer"
                        >
                      ...
                      <div>
                        ...
                        <select
                          value={añoDepto}
                          onChange={(e) => setAñoDepto(e.target.value)}
                          className="block w-full px-3 py-2 bg-slate-50 border border-slate-200 text-[#002147] text-xs font-bold rounded-xl focus:ring-4 focus:ring-blue-50 focus:border-blue-200 transition-all outline-none cursor-pointer"
                        >
                  ...
                  {/* Leyenda Informativa para Histórico */}
                  {periodoDepto === 'historico' && (
                    <div className="flex items-center justify-center p-3 bg-slate-50 border border-dashed border-slate-200 rounded-xl text-center col-span-1 md:col-span-2">
                      <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest leading-normal">
                        Se consultará todo el histórico de remisiones sin límites de fechas.
                      </span>
                    </div>
                  )}

                  {/* Filtro Opcional de Oficina */}
                  <div>
                    <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Oficina Origen</label>
                    <select
                      value={oficinaDepto}
                ...
                <div className="flex-1 w-full lg:w-auto flex flex-col gap-4 text-left">
                  {/* Hilera 1: Fechas y Horas */}
                  ...
                  {/* Hilera 2: Tipo de Denuncia y Destino */}
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                    {/* Tipo de Denuncia */}
                    <div className="md:col-span-2">
              ...
              <div className="flex items-center gap-3 w-full lg:w-auto">
                <button
                  ...
                  disabled={cargando || cargandoObjetos}
                ...
                >
                  {cargando || cargandoObjetos ? <RefreshCcw className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />}
                  {cargando || cargandoObjetos ? 'Buscando...' : 'Consultar'}
                </button>
                <button
          ...
          {/* Resultados Objetos Perdidos */}
          {activeTab === 'objetos_perdidos' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 font-sans text-left">
              {/* Estadísticas de Objetos */}
              {statsObjetos && (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                  {/* Gráfico / Lista de Objetos más perdidos */}
                  <div className="bg-white rounded-2xl shadow-xl shadow-blue-900/5 p-6 border border-slate-100 relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-1 h-full bg-[#002147]"></div>
                    <h3 className="text-xs font-black text-[#002147] uppercase tracking-wider mb-4 flex items-center gap-2">
                      <FilePieChart className="w-4 h-4 text-blue-600" />
                      Objetos Más Extraviados
                    </h3>
                    {statsObjetos.por_tipo?.length > 0 ? (
                      <div className="space-y-3">
                        {statsObjetos.por_tipo.slice(0, 5).map((item: any, idx: number) => (
                          <div key={idx} className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-100">
                            <span className="text-xs font-bold text-slate-600 uppercase tracking-tight truncate max-w-[200px]">{item.label}</span>
                            <span className="px-2 py-0.5 bg-blue-100 text-blue-800 text-[10px] font-black rounded-lg">{item.cantidad}</span>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-wider py-8 text-center">Sin datos estadísticos</p>
                    )}
                  </div>

                  {/* Bancos Emisores más Comunes */}
                  <div className="bg-white rounded-2xl shadow-xl shadow-blue-900/5 p-6 border border-slate-100 relative overflow-hidden">
                    <div className="absolute top-0 left-0 w-1 h-full bg-blue-600"></div>
                    <h3 className="text-xs font-black text-[#002147] uppercase tracking-wider mb-4 flex items-center gap-2">
                      <Briefcase className="w-4 h-4 text-blue-600" />
                      Bancos Emisores Comunes (Tarjetas/Cheques)
                    </h3>
                    {statsObjetos.por_banco?.length > 0 ? (
                      <div className="space-y-3">
                        {statsObjetos.por_banco.slice(0, 5).map((item: any, idx: number) => (
                          <div key={idx} className="flex justify-between items-center bg-slate-50 p-3 rounded-xl border border-slate-100">
                            <span className="text-xs font-bold text-slate-600 uppercase tracking-tight truncate max-w-[200px]">{item.banco.toUpperCase()}</span>
                            <span className="px-2 py-0.5 bg-green-100 text-green-800 text-[10px] font-black rounded-lg">{item.cantidad}</span>
                          </div>
                        ))}
                      </div>
                    ) : (
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-wider py-8 text-center">Sin cheques ni tarjetas</p>
                    )}
                  </div>
                  ...
                  <div className="bg-white rounded-2xl shadow-xl shadow-blue-900/5 p-6 border border-slate-100 relative overflow-hidden">
                    ...
                    <h3 className="text-xs font-black text-[#002147] uppercase tracking-wider mb-4 flex items-center gap-2">
                      ...
                      Estadísticas de Chapas / Patentes
                    </h3>
                    {statsObjetos.chapas?.por_origen?.some((c: any) => c.cantidad > 0) ? (
                      <div className="space-y-4">
                        <div className="grid grid-cols-2 gap-3 text-center">
                          {statsObjetos.chapas.por_origen.map((item: any, idx: number) => (
                            <div key={idx} className="bg-slate-50 p-3 rounded-xl border border-slate-100">
                              <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-1">{item.origen}</span>
                              <span className="text-lg font-black text-[#002147]">{item.cantidad}</span>
                            </div>
                          ))}
                        </div>
                        {statsObjetos.chapas.por_pais?.length > 0 && (
                          <div className="border-t border-slate-100 pt-3">
                            <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest block mb-2">Países Mercosur:</span>
                            <div className="flex flex-wrap gap-2">
                              {statsObjetos.chapas.por_pais.map((item: any, idx: number) => (
                                <span key={idx} className="inline-block px-2.5 py-1 bg-slate-100 text-slate-600 text-[9px] font-black uppercase rounded-lg">
                      ...
                      <p className="text-xs font-bold text-slate-400 uppercase tracking-wider py-8 text-center">Sin chapas registradas</p>
                    )}
                  </div>
                </div>
              )}

              {/* Contenedor Principal: Buscador y Ranking */}
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                {/* Lista / Buscador de Objetos */}
                <div className="lg:col-span-2 bg-white rounded-2xl shadow-xl shadow-blue-900/5 border border-slate-100 overflow-hidden relative">
                  <div className="absolute top-0 left-0 w-1 h-full bg-[#002147]"></div>
                  ...
                  {cargandoObjetos ? (
                    <div className="p-12 text-center text-slate-500 font-bold font-sans">Cargando resultados...</div>
                  ) : datosObjetos.length > 0 ? (
                    <div className="divide-y divide-slate-100 max-h-[600px] overflow-y-auto pr-1">
                      {datosObjetos.map((item: any, idx: number) => {
                        const obj = item.objeto
                        return (
                          <div key={idx} className="p-6 hover:bg-slate-50/50 transition-all space-y-4">
                            <div className="flex flex-wrap items-center justify-between gap-3">
                              <span className="inline-block px-2 py-0.5 bg-[#002147] text-white text-[9px] font-black uppercase rounded">
                                {obj.tipo_label}
                              </span>
                              <Link
                                href={`/ver-denuncia/${item.denuncia_id}`}
                                className="inline-flex items-center gap-1 text-[10px] font-black text-blue-600 hover:text-blue-800 uppercase tracking-widest"
                              >
                                Acta {item.numero_denuncia}/{item.año}
                                <ChevronRight className="w-3 h-3" />
                              </Link>
                            </div>

                            <div className="bg-slate-50 p-4 rounded-xl border border-slate-200/50 text-xs text-slate-700 leading-relaxed font-semibold">
                              ...
                              {obj.tipo === 'cheque' && (
                                <>
                                  ...
                                  {obj.estado === 'Completado' && ` | Importe: ${obj.monto} ${obj.moneda} | Beneficiario: ${obj.beneficiario}`}
                                </>
                              )}
                              {obj.tipo === 'celular' && `Marca/Modelo: ${obj.marca} | N° de línea: ${obj.numero}${obj.imei ? ` | IMEI: ${obj.imei}` : ''}`}
                              {obj.tipo === 'contrato' && `Contrato/Documento: ${obj.descripcion} | Partes: ${obj.partes}`}
                              {obj.tipo === 'chapa_vehiculo' && (
                                <>
                                  ...
                                  <br />
                                  Titular: {obj.registradoNombre === 'recurrente' ? 'Denunciante (Recurrente)' : `${obj.nombreTercero.toUpperCase()} (Doc N° ${obj.documentoTercero})`}
                                </>
                              )}
                              {obj.tipo === 'otro_objeto' && `Descripción: ${obj.descripcion}`}
                            </div>

                            <div className="flex flex-wrap items-center gap-y-2 gap-x-6 text-[10px] font-bold text-slate-400 uppercase tracking-tighter">
                              <span className="flex items-center gap-1 text-slate-500 font-black">
                                <User className="w-3.5 h-3.5 text-slate-400" />
                                {item.denunciante}
                              </span>
                              <span>CI/Doc: {item.denunciante_cedula || '-'}</span>
                ...
                {/* Ranking de Personas que más denuncian */}
                <div className="bg-white rounded-2xl shadow-xl shadow-blue-900/5 border border-slate-100 overflow-hidden relative h-fit">
                  <div className="absolute top-0 left-0 w-1 h-full bg-[#002147]"></div>
                  <div className="px-6 py-5 border-b border-slate-100 bg-slate-50/50 flex items-center gap-3">
                    <div className="p-2 bg-[#002147] rounded-xl">
                      <Users className="w-4 h-4 text-white" />
                  ...
                  {statsObjetos?.por_denunciante?.length > 0 ? (
                    <div className="divide-y divide-slate-50 font-medium">
                      {statsObjetos.por_denunciante.map((row: any, idx: number) => (
                        <div key={idx} className="p-4 hover:bg-slate-50/60 transition-colors flex items-center justify-between gap-4">
                          <div className="min-w-0">
                            <span className="text-[10px] font-black text-[#002147] uppercase tracking-tight block truncate">
                              {row.nombre}
                            </span>
          ...
          {activeTab === 'departamentos' && (
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 font-sans text-left">
              {/* Contenedor Principal: Ranking de Departamentos */}
              <div className="max-w-4xl mx-auto bg-white rounded-2xl shadow-xl shadow-blue-900/5 border border-slate-100 overflow-hidden relative">
                <div className="absolute top-0 left-0 w-1 h-full bg-[#002147]"></div>
                <div className="px-6 py-5 border-b border-slate-100 bg-slate-50/50 flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h3 className="text-[11px] font-black text-[#002147] uppercase tracking-widest">Denuncias Remitidas por Departamento</h3>
                      <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">
                        Ranking de dependencias especializadas con denuncias remitidas
                      </p>
                    </div>
                  </div>
                  {datosDepartamentos.length > 0 && (
                    <span className="px-3 py-1 bg-blue-50 text-blue-700 font-black text-[10px] uppercase rounded-xl tracking-wider">
                      {datosDepartamentos.length} departamentos activos
                    </span>
                  )}
                </div>

                {cargando ? (
                  <div className="p-12 text-center text-slate-500 font-bold font-sans flex flex-col items-center gap-2">
                    <RefreshCcw className="w-6 h-6 animate-spin text-[#002147]" />
                    <span className="text-xs uppercase tracking-widest">Cargando estadísticas...</span>
                  </div>
                ) : error ? (
                  <div className="p-12 text-center text-red-500 font-bold font-sans text-xs uppercase tracking-wider">
                    {error}
                  </div>
                ) : datosDepartamentos.length > 0 ? (
                  <div className="divide-y divide-slate-100">
                    {datosDepartamentos.map((row, idx) => (
                      <div key={idx} className="p-5 hover:bg-slate-50/60 transition-colors flex items-center justify-between gap-6">
                        <div className="flex items-center gap-4 min-w-0">
                          <div className="h-8 w-8 bg-[#002147]/5 text-[#002147] font-black rounded-xl flex items-center justify-center text-xs shrink-0 border border-[#002147]/10">
                            {idx + 1}
                          </div>
                          <div className="min-w-0">
                            <span className="text-xs font-black text-[#002147] uppercase tracking-tight block truncate md:max-w-2xl max-w-[250px]" title={row.departamento}>
                              {row.departamento}
                            </span>
                          </div>
                        </div>
                        <div className="flex items-center gap-3 shrink-0">
                          <div className="w-24 bg-slate-100 rounded-full h-2 hidden sm:block overflow-hidden">
                            <div 
                              className="bg-[#002147] h-2 rounded-full" 
                              style={{ width: `${(row.cantidad / datosDepartamentos[0].cantidad) * 100}%` }}
                            />
                          </div>
                          <span className="px-3 py-1.5 bg-blue-50 text-blue-700 font-black text-xs rounded-xl border border-blue-100">
                            {row.cantidad} {row.cantidad === 1 ? 'remisión' : 'remisiones'}
                          </span>
                        </div>
                      </div>
                    ))}
                  ...
                  <div className="p-16 text-center text-slate-400 font-black uppercase tracking-widest text-xs italic">
                    No se encontraron denuncias remitidas en este período
                  </div>
                )}
              </div>
            </div>
